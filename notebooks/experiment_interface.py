...
class ExperimentPlatformWidget:
    ...
    def _create_widgets(self):
        ...
        self.tab.titles = ["🚀 Experiment Designer", "⚙️ Config Editor", "📊 Results", "📚 Tutorials"]
    ...
    def _create_tutorial_runner(self):
        ...
        self.run_tutorial_btn = widgets.Button(
            description="Run Tutorial",
            button_style='success',
            icon='graduation-cap'
        )
        
        # Tutorial info
        self.tutorial_info = widgets.HTML(
            value="<p>Select a tutorial to see more information.</p>"
        )
        
        # Layout
        controls = widgets.HBox([
            self.tutorial_selector,
            self.install_deps,
            self.run_tutorial_btn
        ])
    ...
    def _on_generate_config(self, button):
        ...
        with self.output:
            ...
            try:
                ...
                if input_data:
                    self._create_dataset(config["dataset"]["name"], config["dataset"]["version"], input_data)
                
                print("✅ Configuration generated successfully!")
                print(f"📝 Config updated in editor tab")
                if input_data:
        ...
        with self.output:
            ...
            try:
                ...
                if missing_fields:
                    ...
                    self.validation_output.value = f"<div style='color: red;'>❌ Missing: {', '.join(missing_fields)}</div>"
                    return
                ...
                if DIRECT_IMPORT:
                    ...
                    try:
                        ...
                        self.validation_output.value = "<div style='color: green;'>✅ Configuration is valid!</div>"
                    except Exception as e:
                    ...
                    self.validation_output.value = "<div style='color: green;'>✅ Basic validation passed!</div>"
                    
            except yaml.YAMLError as e:
                print(f"❌ YAML parsing error: {e}")
                self.validation_output.value = f"<div style='color: red;'>❌ YAML Error: {str(e)}</div>"
            except Exception as e:
                print(f"❌ Validation error: {e}")
    ...
    def _on_tutorial_selected(self, change):
        ...
        selected = change['new']
        self.tutorial_info.value = tutorial_info.get(selected, "<p>Select a tutorial to see more information.</p>")
...
def quick_experiment(module_path: str, 
                    data: List[Dict[str, Any]], 
                    evaluators: List[str] = None,
                    dataset_name: str = "quick_experiment",
                    output_path: str = "data/experiments") -> str:
    """
    ...
    config = {
        ...
        "evaluation": [
            {"id": f"{name}_eval", "name": name, "data_mapping": {}}
