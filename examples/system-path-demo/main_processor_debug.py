...
try:
    ...
    for path in added_paths:
        relative_to_cwd = Path(path).relative_to(current_cwd) if current_cwd in Path(path).parents or current_cwd == Path(path) else "Not relative to CWD"
        print(f"   • {path}")
    ...
    print(f"📍 Current sys.path entries containing 'system-path':")
