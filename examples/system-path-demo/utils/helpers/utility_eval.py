    ...
    def evaluate(self, rows: list[DataModelRow]) -> EvaluatorOutput:
        ...
        for row in rows:
            ...
            has_greeting_response = any(
                word in input_text.lower() for word in ["hello", "hi", "hey"]
            ) and any(word in response.lower() for word in ["hello", "hi", "hey"])
        ...
        return EvaluatorOutput(name=self.config.name, summary=summary, per_row=per_row)
