...
class AdvancedCoherenceEvaluator(BaseEvaluator):
    ...
    def evaluate(self, rows: list[DataModelRow]) -> EvaluatorOutput:
        ...
        for row in rows:
            ...
            has_logical_flow = bool(re.search(r'\b(first|then|next|finally|therefore|because|since)\b', output_text))
            consistent_tone = not bool(re.search(r'[.!?]\s*[a-z]', output_text))  # Consistent capitalization
