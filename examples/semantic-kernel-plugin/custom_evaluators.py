...
class ResponseLengthEvaluator(BaseEvaluator):
    ...
    def evaluate(self, rows: list[DataModelRow]) -> EvaluatorOutput:
        ...
        for row in rows:
            ...
            per_row_scores[row.id] = {"length": float(length), "length_score": score}
        ...
        summary = {
            ...
            "average_length_score": statistics.mean(
                [s["length_score"] for s in per_row_scores.values()]
            )
            if per_row_scores
            else 0.0,
        }
...
class SentimentEvaluator(BaseEvaluator):
    ...
    def evaluate(self, rows: list[DataModelRow]) -> EvaluatorOutput:
        ...
        total = len(rows)
        summary = {
            "average_sentiment_score": statistics.mean(
                [s["sentiment_score"] for s in per_row_scores.values()]
            )
            if per_row_scores
            else 0.0,
