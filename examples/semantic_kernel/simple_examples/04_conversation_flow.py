...
class ConversationAgent:
    ...
    def get_conversation_analysis(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "conversation_started": self.conversation_metadata["start_time"].isoformat() if self.conversation_metadata["start_time"] else None
        }
...
async def main():
    ...
    for i, message in enumerate(conversation_script, 1):
        ...
        if result["success"]:
            ...
            print(f"Duration: {result['duration_ms']:.1f}ms | Context: {result['context_length']} messages")
        else:
    ...
    print(f"Duration: {analysis['duration_seconds']:.1f} seconds" if analysis['duration_seconds'] else "Duration: N/A")
