...
async def main():
    ...
    print(
        f"Plugins used: {', '.join(summary['plugins_used']) if summary['plugins_used'] else 'None'}"
    )
    print(
        f"Functions used: {', '.join(summary['functions_used']) if summary['functions_used'] else 'None'}"
    )
    ...
    if summary["failed_calls"] > 0:
        ...
        for call in summary["all_calls"]:
            if not call.get("success", True):
                print(
                    f"  {call.get('plugin_name')}.{call.get('function_name')}: {call.get('error', 'Unknown error')}"
                )
