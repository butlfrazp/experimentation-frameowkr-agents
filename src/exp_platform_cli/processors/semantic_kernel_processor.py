...
class SemanticKernelProcessor(BaseProcessor[dict]):
    ...
    def _extract_agent_interaction(self, data_output: dict[str, Any]) -> AgentInteraction | None:
        ...
        if "kernel_result" in data_output or "function_result" in data_output:
            ...
            interaction = AgentInteraction(
                interaction_id=interaction_id, framework=AgentFramework.SEMANTIC_KERNEL, metadata={}
            )
    ...
    def _extract_sk_trace(self, data_output: dict[str, Any]) -> SemanticKernelTrace | None:
    ...
    def _parse_function_call(self, call_data: dict[str, Any]) -> ToolCallDetails | None:
    ...
    def _parse_kernel_invocation(self, invocation_data: dict[str, Any]) -> ToolCallDetails | None:
        ...
        return ToolCallDetails(
            ...
            error_message=str(invocation_data.get("exception"))
            if invocation_data.get("exception")
            else None,
            metadata={
    ...
    def _extract_conversation_history(self, data_output: dict[str, Any]) -> list[AgentMessage]:
    ...
    def _parse_chat_message(self, msg_data: dict[str, Any]) -> AgentMessage | None:
    ...
    def _process_sk_metadata(self, row: DataModelRow) -> None:
        ...
        if "semantic_function" in metadata:
            ...
            if isinstance(func_info, dict):
                ...
                row.metadata["sk_function_parameters"] = list(
                    func_info.get("parameters", {}).keys()
                )
    ...
    def _parse_plugin_execution(self, execution_data: dict[str, Any]) -> ToolCallDetails | None:
