    ...
    def _get_ai_project_client(self):
        ...
        if self._ai_project_client is None:
            try:
                ...
                log.error(
                    "Azure AI Projects SDK not installed. Install with: pip install azure-ai-projects"
                )
                raise
    ...
    def evaluate(
        ...
        *,
        ...
        try:
            ...
            if data_id is None:
                data_id = self._upload_dataset(
                    client, dataset_name, dataset_version, rows_list, experiment_id
                )
            ...
            for evaluator_config in evaluators:
                try:
                    run_result = self._run_cloud_evaluation(
                        client, experiment_id, data_id, evaluator_config, config
                    )
            ...
            self._collect_and_save_results(
                evaluation_runs, experiment_id, dataset_name, dataset_version, rows_list, config
            )
    ...
    def _run_cloud_evaluation(
        ...
        try:
            ...
            evaluator_cfg = EvaluatorConfiguration(
                ...
                evaluator="relevance"
                if "relevance" in evaluator_config.name.lower()
                else "groundedness",
            )

            # Configure input data
            input_data = InputData(
                data_id=data_id, data_mapping=evaluator_config.data_mapping or {}
            )
    ...
    def _create_placeholder_results(self, evaluation_run, rows: list[DataModelRow]):
        ...
        for row in rows:
            ...
            row.evaluation_results[f"cloud_{evaluation_run.display_name}:score"] = EvaluationResult(
                ...
                metadata={
                    "evaluator": f"cloud_{evaluation_run.display_name}",
                    "run_id": evaluation_run.id,
                },
            )

    def _save_cloud_results_locally(
        self, output_dir: Path, rows: list[DataModelRow], config: ExperimentConfig, evaluation_runs
    ):
