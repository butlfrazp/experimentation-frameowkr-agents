...
class LocalEvaluationService(EvaluationService):
    ...
    def evaluate(
        ...
        *,
        ...
        for evaluator in evaluator_instances:
            ...
            try:
                ...
                for metric_name, metric_value in result.summary.items():
                    ...
                    if error:
                        evaluation_errors.setdefault(evaluator_name, {}).setdefault(
                            "summary_errors", {}
                        )[metric_name] = error
                        # Store non-numeric summary in separate bucket
                        non_numeric_metrics.setdefault(evaluator_name, {}).setdefault(
                            "summary", {}
                ...
                for row in rows_list:
                    ...
                    for metric_name, metric_value in metrics.items():
                        ...
                        if error:
                            ...
                            evaluation_errors.setdefault(evaluator_name, {}).setdefault(
                                "per_row_errors", {}
                            ).setdefault(row.id, {})[metric_name] = error
                            ...
                            logger.debug(
                                f"Stored non-numeric metric {key}: {metric_value} ({type(metric_value).__name__})"
                            )
                        else:
        ...
        successful_evaluators = len(evaluator_instances) - len(
            [e for e in evaluation_errors.values() if "execution_error" in e]
        )
        logger.info(
            f"Evaluation completed: {successful_evaluators}/{len(evaluator_instances)} evaluators successful"
        )

        if evaluation_errors:
            logger.warning(
                f"Evaluation issues found: {len(evaluation_errors)} evaluators had errors"
            )
            for evaluator_name, errors in evaluation_errors.items():
                if "execution_error" in errors:
                    logger.error(
                        f"  {evaluator_name}: Execution failed - {errors['execution_error']['error']}"
                    )
                if "summary_errors" in errors:
                    logger.debug(
                        f"  {evaluator_name}: {len(errors['summary_errors'])} summary metric conversion errors"
                    )
                if "per_row_errors" in errors:
                    total_row_errors = sum(
                        len(row_errors) for row_errors in errors["per_row_errors"].values()
                    )
                    logger.debug(
                        f"  {evaluator_name}: {total_row_errors} per-row metric conversion errors"
                    )
    ...
    def _convert_to_numeric(self, value: Any, context: str) -> tuple[float, str | None]:
        ...
        try:
            ...
            return (
                numeric_val,
                f"Value {value} ({type(value).__name__}) converted to {numeric_val} in {context}",
            )
        except (TypeError, ValueError) as e:
            return (
                0.0,
                f"Value {value} ({type(value).__name__}) could not be converted to float in {context}: {e}",
            )
