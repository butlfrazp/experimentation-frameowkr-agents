    ...
    for evaluator_name in available_evaluators:
        ...
        try:
            ...
            for key, value in result.summary.items():
                ...
                print(f"   • {key}: {value} ({value_type}) {'✅' if is_numeric else '❌ NOT NUMERIC'}")
                
                if not is_numeric:
                    print(f"     ⚠️  WARNING: '{key}' is not numeric - this may cause float conversion errors!")
            
            # Check per-row metrics
            if result.per_row:
                print(f"📋 Per-row metrics (sample from first row):")
