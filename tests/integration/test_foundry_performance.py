...
''')
        ...
        if failed_results:
            ...
            for failed in failed_results:
                print(
                    f"  - {failed['config_file']}: {failed.get('stderr', failed.get('error', 'Unknown error'))}"
                )
        ...
        if result.returncode == 0:
            ...
            if results_dir.exists():
                ...
                if data_file.exists():
                    ...
                    assert (
                        file_size < max_expected_size
                    ), f"Results file too large: {file_size} bytes"
        else:
...
""")
        ...
        for eval_count in evaluator_counts:
            ...
            for i in range(eval_count):
                ...
                evaluators.append(
                    {"id": f"eval_{i}", "name": eval_names[i % len(eval_names)], "data_mapping": {}}
                )
            ...
            print(
                f"Evaluators: {eval_count}, Time: {execution_time:.2f}s, Success: {result.returncode == 0}"
            )
        ...
        if len(successful_results) >= 2:
            ...
            if len(times) >= 2:
                ...
                print(
                    f"Scalability ratio: {time_ratio:.2f}x time for {evaluator_ratio:.2f}x evaluators"
                )

                # Should scale sub-linearly (good) or at most linearly (acceptable)
                assert (
                    time_ratio <= evaluator_ratio * 1.5
                ), f"Poor scalability: {time_ratio:.2f}x time for {evaluator_ratio:.2f}x evaluators"
...
""")
        ...
        if result.returncode == 0:
            ...
            if results_dir.exists():
                ...
                assert (
                    avg_time_per_experiment < 300
                ), f"Too slow: {avg_time_per_experiment:.1f}s per experiment"
        else:
