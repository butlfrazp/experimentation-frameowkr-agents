...
class TestFoundryExperimentExecution:
    ...
    def test_foundry_experiment_basic(self, foundry_experiment_config, tmp_path):
        ...
        if result.returncode == 0:
            assert "experiment completed" in result.stdout.lower() or "success" in result.stdout.lower()
        else:
    ...
    def test_foundry_experiment_with_multiple_evaluators(self, foundry_config, sample_test_data, tmp_path):
...
''')
    ...
    def test_foundry_experiment_error_handling(self, foundry_config, tmp_path):
        ...
        config = {
            ...
            "evaluation": [
                {"id": "test_eval", "name": "conversation_quality", "data_mapping": {}}
            ],
            "local_mode": False,
            **foundry_config.get_foundry_config()
        }
